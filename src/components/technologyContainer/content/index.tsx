              ...
              <div className='flex flex-row lg:flex-col   gap-8  h-full'>
                {
                  contentItems.map(item => (
                    <div key={item.id} onClick={() => setCurrentContent(item.name)} className={classNames(' w-10 h-10 md:w-[60px] md:h-[60px] lg:w-20 lg:h-20 bg-white flex items-center justify-center text-black rounded-full',{'bg-transparent border text-white' : item.name !== currentContent})}>
